...
form_class = uic.loadUiType("Import_purchases.ui")[0]

class MyWindowClass(QtWidgets.QDialog, form_class):
	def __init__(self, parent=None):
		
		QtWidgets.QDialog.__init__(self, parent)
		list_ingredients = ['BRUSSEL SPR', 'CRKLE CARR CONV', 'BABY ORG CARR',
	...
	def genera_archivo_csv(self, list_keys,year):
		...
		datos_import_Purchases =pd.read_csv('importpurchases.csv', index_col = 0, encoding = 'utf-8')
		datos_import_Purchases.apply(lambda x: pd.lib.infer_dtype(x.values))
		num_datos = int(datos_import_Purchases['indice'].count())
		datos_import_Purchases.index = range(datos_import_Purchases.shape[0])
		indice_archivo = list(datos_import_Purchases.indice)
		...
		for i, valor in enumerate(valores):
			...
			datos_import_Purchases.loc[int(nuevo_valor)]= [
				...
				valor[36], valor[37]]
		try: 
			datos_import_Purchases.to_csv('importpurchases.csv',  encoding = 'utf-8',) 
